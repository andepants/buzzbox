...
- **Processing:**
  1. Categorize message (fan/business/spam/urgent) - GPT-4o-mini
  2. Analyze sentiment (positive/negative/urgent/neutral) - GPT-4o-mini
  3. Score opportunity (0-100 if business) - GPT-4o-mini
- **Output:** Updates message in RTDB with AI metadata
- **Latency:** <1s (parallel processing, GPT-4o-mini is FAST!)
- **iOS:** No code needed - automatic!

**Function 2: FAQ Auto-Responder (Feature 3)**
- **Trigger:** HTTP callable function
- **Processing:**
  1. Fetch all 15 FAQs from Firestore
  2. Send FAQs + user question to GPT-4o-mini as context
  3. GPT-4o-mini determines if question matches an FAQ
  4. If match → Return FAQ answer verbatim
- **Output:** `{ isFAQ: true, answer: "..." }` or `{ isFAQ: false }`
- **Latency:** <1s (GPT-4o-mini is blazing fast!)
- **iOS:** Calls function, auto-sends if FAQ match
- **Simple:** No embeddings, no vector indexes needed!
...
- **Processing:**
  ...
  3. Call OpenAI GPT-4o-mini with:
...
- **Latency:** <3s (GPT-4o-mini is MUCH faster than GPT-4!)
...
### Story 6.3: FAQ Auto-Responder Cloud Function (Feature 3) (30 min)

**As a fan, I want instant answers to common questions so I don't have to wait for Andrew.**

**SIMPLIFIED APPROACH:** With only 15 FAQs, we send all FAQs to GPT-4 as context. No embeddings, no vector search, no complexity.

**FAQ Library Setup (Manual):**

Create 15 FAQs in Firestore manually via Firebase Console:
...
export const checkFAQ = onCall(async (request) => {
  ...
  try {
    ...
    // 3. Ask GPT-4o-mini to match and respond
    const completion = await openai.chat.completions.create({
      model: 'gpt-4o-mini',
      messages: [
        {
...
If no good match, respond with exactly: NO_MATCH`
    ...
    // 4. Check if we got a match
    if (response === 'NO_MATCH') {
      return { isFAQ: false } as FAQResponse;
    }

    // 5. Find which FAQ was matched (for logging)
...
**Acceptance Criteria:**
- [ ] 15 FAQs created in Firestore manually (no embeddings needed!)
- [ ] `checkFAQ` function fetches all FAQs and sends to GPT-4o-mini
- [ ] GPT-4o-mini matches questions semantically (handles different wording)
- [ ] Returns exact FAQ answer when matched
...
- [ ] Response time <1s (GPT-4o-mini is FAST!)
- [ ] No vector indexes needed
- [ ] No embedding generation step needed

**Estimate:** 30 min (reduced from 1.5 hours - no embedding complexity!)
...
- [ ] GPT-4o-mini generates 3 distinct reply options
- [ ] iOS shows smart reply picker with 3 options
- [ ] Selecting a draft populates message composer (editable)
- [ ] Response time <3s (GPT-4o-mini is 3x faster than GPT-4!)
...
| 6.3 | FAQ Cloud Function (Feature 3) **SIMPLIFIED** | 30 min |
...
- **SIMPLIFIED FAQ (Story 6.3):** Removed vector embeddings/indexes - just send 15 FAQs to GPT-4 (saved 1 hour!)
- **Added:** Production-only deployment (no separate dev environment)

**Realistic estimate: 10.25 hours** (vs original n8n approach: 12.5 hours, vector embeddings: 11.25 hours)
...
**No indexes needed!** - FAQs are fetched and sent to GPT-4 as context.
...
- GPT-4o-mini's speed helps offset cold starts (3x faster than GPT-4)
...
**Mitigation:**
- Use GPT-4o-mini for ALL tasks (15x cheaper than GPT-4, 2x cheaper than GPT-3.5-turbo!)
- No embeddings = no extra costs
- Set budget alerts in OpenAI ($10, $20)
- Monitor per-feature costs
- Estimated: $5-10 for testing/demo (GPT-4o-mini is VERY cheap!)
...
### Phase 3: FAQ Auto-Responder (30 min) **SIMPLIFIED**
14. Create 15 FAQs in Firestore manually (just question, answer, category - no embeddings!)
15. Write checkFAQ Cloud Function (fetches all FAQs, sends to GPT-4)
16. Deploy and test FAQ matching with real messages
17. Add AIService.checkFAQ() in iOS
18. Integrate FAQ auto-response in message receiving
19. Add AI-generated badge to UI

### Phase 4: Context-Aware Smart Replies (2 hours)
20. Create creator profile in Firestore manually (use template from PRD)
21. Write generateSmartReplies Cloud Function
22. Deploy and test with real conversation context
23. Create SmartReplyPickerView UI component
24. Add "Draft Reply" button to MessageThreadView
25. Test end-to-end with real production conversations

### Phase 5: Error Handling & Settings (1.5 hours)
26. Add timeout and error handling to Cloud Functions
27. Deploy and test error scenarios in production
28. Add graceful degradation to iOS
29. Create AI Settings UI
30. Wire up settings toggles

### Phase 6: Security & Polish (1 hour)
31. Deploy Firestore security rules to production
32. Test all 5 AI features end-to-end in production
33. Verify latency meets targets
34. Test error handling (network failures, timeouts)
35. Polish UI animations and loading states
